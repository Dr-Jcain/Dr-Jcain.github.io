...
def main() -> int:
    ...
    for course_dir in sorted(p for p in COURSES_DIR.iterdir() if p.is_dir()):
        ...
        strict_lesson_slugs = bool(course.get("strictLessonSlugs", False))
        ...
        for lesson_dir in sorted(p for p in course_dir.iterdir() if p.is_dir() and not p.name.startswith(".")):
            ...
            # Algunas materias pueden conservar carpetas históricas o borradores.
            # Si strictLessonSlugs=true, sólo se publican las carpetas cuyo slug
            # está declarado explícitamente en course.json.
            if strict_lesson_slugs and lesson_dir.name not in by_slug:
                continue

            meta = read_lesson_meta(index_path)
            number = meta["number"]
            matched = None
            if strict_lesson_slugs:
